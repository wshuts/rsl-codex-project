...
| 6 | Irethi Coronet | 3-star Epic, +9 | Tomb Lord | Direct HH match; repeated debuff placement can build the relic's damage bonus naturally. |
| 7 | Gilded Dragonstone | 3-star Epic, +9 | Visix the Unbowed | One of only two observed HH matches for this relic; her DEF-based, hit-taking profile is coherent with its stacking DEF effect. |
| 8 | Gilded Dragonstone | 3-star Epic, +9 | Morag Bronzelock | The other observed HH match. Use a 3-star copy before considering any 2-star duplicates. |
| 9 | Demonic Effigy | 3-star Epic, +9 | Duchess Lilitu | Direct HH match and a coherent reviver pairing, though the relic is only C-tier and should remain below the upgrades above. |

## Strong Alternatives

Use these when the first-choice champion has a mode-specific conflict.

| Relic | Alternative HH-matched champions | Decision note |
|---|---|---|
| Wand of Submission | Warlord, Duchess Lilitu, Lyssandra, Maulie Tankard, Sun Wukong | Warlord is the strongest alternative candidate, but his current Orb assignment is already an HH match. |
| Aspect of Siroth | Warlord | Prefer Warlord over Galleus if Arena survival is the explicit goal and losing Orb is acceptable. |
| Golden Elixir | Artak, Firrol, Frozen Banshee, Urogrim, Vogoth | Urogrim and Vogoth are especially plausible tanky-use candidates; Artak is also attractive for solo PvE. |
| Orb of Transfixion | Artak, Basher, Firrol, Old Ghrukkus | Basher is a clean control/debuffer alternative. Avoid Seeker if he is speed-tuned. |
| Irethi Coronet | Coldheart, Leminisi, Ninja, Astralon, Fenax, Kael, Maneater, Martyr, Mikey | Choose a frequent debuffer whose actual build also benefits from ATK and ACC. |
| Demonic Effigy | Maulie Tankard | Use only as filler; local tier notes rate this relic C-tier. |

## Existing Assignments to Review

These are not automatic removals. They are current pairings absent from the observed HH recommendation for that champion, while a roster-matched alternative exists.
...
| Rector Drath | Demonic Effigy | Wand, Journal, and Perched Shadow are observed matches; no such spare is currently catalogued except Wand, which is higher priority on Arbiter. |
| Madame Serris | Molten Deathbell | No Madame-specific HH screen is present in the dataset, so this is an evidence gap rather than a demonstrated mismatch. |
| Criodan | Molten Deathbell | Journal or Perched Shadow are the observed matches, but neither is in the unequipped high-rarity pool. |

The Golden Elixirs on Yakarl and Whisper are also unsupported by the current HH sample, but neither champion has a processed recommendation screen. Treat those as unknown, not mismatches.

## Reserve and Food Decisions

- Preserve all three Legendary relics: the Wand and both Aspects.
- Preserve the 3-star copies selected in the first-pass table.
- Preserve at least one best copy of every relic whose champion recommendation coverage is incomplete.
